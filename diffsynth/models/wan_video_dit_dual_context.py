...
# class KeyframeCrossAttention(nn.Module):
#     def __init__(self, dim: int, num_heads: int, eps: float = 1e-6, has_image_input: bool = False):
#         super().__init__()
#         self.dim = dim
#         self.num_heads = num_heads
...
#         If has_image_input=True, use original Wan order:
#             y[:, :257]  -> image clip tokens
#             y[:, 257:]  -> text tokens

#         For your task:
#             text tokens should already contain:
#                 global_prompt + local_prompt_i
#         """
#         if self.has_image_input:
#             img = y[:, :257]
#             ctx = y[:, 257:]
...
#         x = rearrange(x, "b (f hw) c -> (b f) hw c", f=frame_num)
...
class KeyframeCrossAttention(nn.Module):
    ...
    def forward(self, x: torch.Tensor, y: torch.Tensor):
        ...
        y:
            [B, F, L, C]     # recommended batched format
            or
            [F, L, C]        # fallback for single-sample inference
        ...
        if y.dim() == 3:
            # [F, L, C] -> [B, F, L, C]
            f = y.shape[0]
            y = y.unsqueeze(0).expand(b, -1, -1, -1)
        elif y.dim() == 4:
            # [B, F, L, C]
            assert y.shape[0] == b, f"batch mismatch: x B={b}, y B={y.shape[0]}"
            f = y.shape[1]
        else:
            raise ValueError(f"bad context shape: {y.shape}")

        assert n % f == 0, f"x tokens {n} not divisible by frame slots {f}"
        hw = n // f

        # x: [B, F*HW, C] -> [B*F, HW, C]
        x = rearrange(x, "b (f hw) c -> (b f) hw c", f=f, hw=hw)

        # y: [B, F, L, C] -> [B*F, L, C]
        y = rearrange(y, "b f l c -> (b f) l c")
        ...
        # [B*F, HW, C] -> [B, F*HW, C]
        x = rearrange(x, "(b f) hw c -> b (f hw) c", b=b, f=f, hw=hw)

        return self.o(x)



##### DUAL-CONTEXT START: bounded learnable scales and branch initialization helpers

def _inverse_bounded_sigmoid(value: float, max_value: float) -> float:
    """Return z such that max_value * sigmoid(z) == value."""
    if not (0.0 < value < max_value):
        raise ValueError(
            f"value must satisfy 0 < value < max_value, got value={value}, max_value={max_value}"
        )
    ratio = value / max_value
    return math.log(ratio / (1.0 - ratio))


def initialize_dual_context_from_global(model: nn.Module):
    """
    Copy pretrained global cross-attention parameters into each local branch.

    Call once after loading the official Wan checkpoint and before loading a
    dual-context training checkpoint.
    """
    copied_blocks = 0
    with torch.no_grad():
...
##### DUAL-CONTEXT START: global/local attention branches with bounded learnable scales
class DiTBlock(nn.Module):
    def __init__(
        ...
        # Original Wan global branch: shared task/scene prompt + CLIP image tokens.
        self.cross_attn = CrossAttention(
            dim, num_heads, eps, has_image_input=has_image_input
        )

        # New local branch: each latent frame only attends to its own local prompt.
        self.local_cross_attn = KeyframeCrossAttention(
            dim, num_heads, eps, has_image_input=False
        )

        # Both branch scales are learnable and bounded in (0, max_context_scale).
        self.max_context_scale = float(max_context_scale)
        self.global_gate = nn.Parameter(
            torch.tensor(
                _inverse_bounded_sigmoid(global_scale_init, self.max_context_scale),
                dtype=torch.float32,
            )
        )
        self.local_gate = nn.Parameter(
            torch.tensor(
                _inverse_bounded_sigmoid(local_scale_init, self.max_context_scale),
                dtype=torch.float32,
            )
        )
    ...
    def context_scales(self):
        global_scale = self.max_context_scale * torch.sigmoid(self.global_gate)
        local_scale = self.max_context_scale * torch.sigmoid(self.local_gate)
        return global_scale, local_scale

    def forward(self, x, global_context, local_context, t_mod, freqs):
        ...
        # Parallel two-branch conditioning from the same normalized hidden state.
        context_input = self.norm3(x)
        global_out = self.cross_attn(context_input, global_context)
        global_scale, local_scale = self.context_scales()

        if local_context is None:
            # Compatibility path for ordinary Wan calls.
            x = x + global_out
        else:
            local_out = self.local_cross_attn(context_input, local_context)
            x = (
                x
                + global_scale.to(dtype=x.dtype) * global_out
                + local_scale.to(dtype=x.dtype) * local_out
            )
...
class WanModel(torch.nn.Module):
    ...
    def forward(self,
                ...
                ##### DUAL-CONTEXT START: separate global and local text conditions
        ...
        ##### DUAL-CONTEXT START: embed and organize two independent text branches
        # Backward compatibility: old callers may still pass `context`.
        if global_context is None:
            global_context = context
        if global_context is None:
            raise ValueError("global_context (or legacy context) must be provided")

        # [B, Lg, text_dim] -> [B, Lg, dim]
        global_context = self.text_embedding(global_context)

        # [B, F, Ll, text_dim] -> [B, F, Ll, dim]
        if local_context is not None:
            local_context = self.text_embedding(local_context)

        if self.has_image_input:
            x = torch.cat([x, y], dim=1)  # (b, c_x + c_y, f, h, w)
            clip_embdding = self.img_emb(clip_feature)
            # Preserve original Wan ordering only in the global branch.
            global_context = torch.cat([clip_embdding, global_context], dim=1)
        ##### DUAL-CONTEXT END
        ...
        for block in self.blocks:
            if self.training:
                x = gradient_checkpoint_forward(
                    ...
                    ##### DUAL-CONTEXT START: pass both branches through checkpointing
                    x, global_context, local_context, t_mod, freqs
                    ##### DUAL-CONTEXT END
                )
            else:
                ##### DUAL-CONTEXT START
                x = block(x, global_context, local_context, t_mod, freqs)
                ##### DUAL-CONTEXT END
