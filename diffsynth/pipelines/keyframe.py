...
class WanVideoUnit_PromptEmbedder(PipelineUnit):
    ...
    def process(self, pipe: WanVideoPipeline, prompt, frame_prompts, num_slots, positive) -> dict:
        ...
        # frame_prompts = 每个 keyframe slot 的 local prompt，包含第 0 帧
        # output context: [1, F, 512, text_dim]
        if num_slots is not None:
            if positive and frame_prompts is not None:
                slot_prompts = [
                    (
                        f"Current keyframe: {frame_prompt}\n"
                        f"Global task and scene: {prompt}"
                    )
                    for frame_prompt in frame_prompts
                ]
            else:
                # negative branch:
                # 这里的 prompt 实际来自 negative_prompt
                slot_prompts = [prompt] * num_slots

            prompt_emb = self.encode_prompt(pipe, slot_prompts)  # [F, 512, text_dim]
            prompt_emb = prompt_emb.unsqueeze(0)                 # [1, F, 512, text_dim]
            return {"context": prompt_emb}

        # Original Wan mode:
        # prompt: str 或 list[str]
        # context: [B, 512, text_dim]
        prompt_emb = self.encode_prompt(pipe, prompt)
        return {"context": prompt_emb}
