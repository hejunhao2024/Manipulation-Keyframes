...
##### DUAL-CONTEXT START: use dual-context DiT definitions and retrofit helpers
from ..models.wan_video_dit_dual_context import (
    WanModel,
    DiTBlock as DualContextDiTBlock,
    CrossAttention as DualContextGlobalCrossAttention,
    KeyframeCrossAttention as DualContextLocalCrossAttention,
...
##### DUAL-CONTEXT START: retrofit a pretrained Wan model after ModelPool loading
def enable_dual_context_on_pretrained_wan(
    model: torch.nn.Module,
    global_scale_init: float = 1.0,
    local_scale_init: float = 1.0,
    max_context_scale: float = 2.0,
):
    """
    Upgrade a pretrained Wan model in-place.

    ModelPool may construct the original WanModel class before this pipeline sees
    it, so importing a new class alone is not sufficient. This helper:
      1) preserves/copies the pretrained global cross-attention weights;
      2) adds a frame-wise local cross-attention branch;
      3) adds bounded learnable global/local gates;
      4) binds the dual-context block forward implementation.

    Call this immediately after fetching the pretrained DiT, and before loading a
    trained dual-context checkpoint.
    """
    if model is None:
        return None

    gate_dtype = next(model.parameters()).dtype
    gate_device = next(model.parameters()).device

    for wrapped_block in model.blocks:
        ##### DUAL-CONTEXT: unwrap DiffSynth VRAM/offload wrapper.
        block = getattr(wrapped_block, "module", wrapped_block)

        # Build a true shared global branch. The previous keyframe model may have
        # used KeyframeCrossAttention here; the parameter layout is compatible.
        if not isinstance(block.cross_attn, DualContextGlobalCrossAttention):
            old_global = block.cross_attn
            has_image_input = bool(getattr(old_global, "has_image_input", False))
            eps = float(getattr(getattr(old_global, "norm_q", None), "eps", 1e-6))
            new_global = DualContextGlobalCrossAttention(
                dim=block.dim,
                num_heads=block.num_heads,
                eps=eps,
                has_image_input=has_image_input,
            ).to(device=gate_device, dtype=gate_dtype)
            new_global.load_state_dict(old_global.state_dict(), strict=False)
            block.cross_attn = new_global

        # Create local branch only once. Copy all compatible text-attention
        # weights from the pretrained global branch.
        if not hasattr(block, "local_cross_attn"):
            eps = float(getattr(block.cross_attn.norm_q, "eps", 1e-6))
            block.local_cross_attn = DualContextLocalCrossAttention(
                dim=block.dim,
                num_heads=block.num_heads,
                eps=eps,
            ...
            global_state = block.cross_attn.state_dict()
            local_state = block.local_cross_attn.state_dict()
            compatible = {
                name: tensor
                for name, tensor in global_state.items()
                if name in local_state and local_state[name].shape == tensor.shape
            }
            block.local_cross_attn.load_state_dict(compatible, strict=False)

        block.max_context_scale = float(max_context_scale)

        global_gate_init = torch.tensor(
            _inverse_bounded_sigmoid(global_scale_init, max_context_scale),
            dtype=torch.float32,
            device=gate_device,
        )
        local_gate_init = torch.tensor(
            _inverse_bounded_sigmoid(local_scale_init, max_context_scale),
            dtype=torch.float32,
            device=gate_device,
        )
        if not hasattr(block, "global_gate"):
            block.global_gate = torch.nn.Parameter(global_gate_init)
        else:
            block.global_gate.data.copy_(global_gate_init)
        if not hasattr(block, "local_gate"):
            block.local_gate = torch.nn.Parameter(local_gate_init)
        else:
            block.local_gate.data.copy_(local_gate_init)

        ##### DUAL-CONTEXT: bind helper and forward methods to the retrofitted block.
        block.context_scales = types.MethodType(
            DualContextDiTBlock.context_scales,
            block,
        )
        block.forward = types.MethodType(
            DualContextDiTBlock.forward,
            block,
        )

    model.dual_context_enabled = True
    return model
##### DUAL-CONTEXT END
...
class WanVideoPipeline(BasePipeline):
    ...
    def from_pretrained(
        ...
        dual_context_global_scale_init: float = 1.0,
        dual_context_local_scale_init: float = 1.0,
        dual_context_max_context_scale: float = 2.0,
    ):
        # Redirect model path
        ...
        if isinstance(dit, list):
            ...
            pipe.dit = dit
        ##### DUAL-CONTEXT START: upgrade pretrained DiT(s) before any custom checkpoint load
        enable_dual_context_on_pretrained_wan(
            pipe.dit,
            global_scale_init=dual_context_global_scale_init,
            local_scale_init=dual_context_local_scale_init,
            max_context_scale=dual_context_max_context_scale,
        )
    ...
    def __call__(
        ...
        if frame_prompts is not None:
            # slot 0 = input_image / initial frame
            # slot 1..K = future keyframes
...
class WanVideoUnit_PromptEmbedder(PipelineUnit):
    ##### DUAL-CONTEXT START: encode global and frame-wise local prompts separately
    def __init__(self):
        super().__init__(
            seperate_cfg=True,
            input_params_posi={
            ...
            output_params=("global_context", "local_context"),
            onload_model_names=("text_encoder",)
        )
    ...
    def process(self, pipe: WanVideoPipeline, prompt, frame_prompts, num_slots, positive) -> dict:
        pipe.load_models_to_device(self.onload_model_names)

        # Shared task/scene condition: [B=1, L, text_dim].
        global_context = self.encode_prompt(pipe, prompt)

        if num_slots is None:
            # Preserve ordinary Wan behavior outside keyframe mode.
            return {"global_context": global_context, "local_context": None}

        if positive and frame_prompts is not None:
            # Each slot receives only its local stage description.
            local_context = self.encode_prompt(pipe, frame_prompts)  # [F, L, text_dim]
        else:
            # CFG negative branch: repeat the negative prompt for every slot so
            # global/local branches keep identical batch and temporal shapes.
            local_context = self.encode_prompt(pipe, [prompt] * num_slots)

        local_context = local_context.unsqueeze(0)  # [1, F, L, text_dim]
        return {
...
class WanVideoUnit_CfgMerger(PipelineUnit):
    def __init__(self):
        ...
        ##### DUAL-CONTEXT START: merge both condition branches for CFG
        self.concat_tensor_names = ["global_context", "local_context", "clip_feature", "y", "reference_latents"]
        ##### DUAL-CONTEXT END
...
def model_fn_wan_video(
    ...
    ##### DUAL-CONTEXT START: separate text conditions; context kept as legacy alias
    ...
    **kwargs,
    ...
    ##### DUAL-CONTEXT START: legacy calls map context to the global branch
    ...
    ##### DUAL-CONTEXT START: independently embed and batch-align both branches
    if global_context is None:
        raise ValueError("global_context is required for Wan dual-context inference/training")

    global_context = dit.text_embedding(global_context)  # [B, Lg, C]

    if local_context is not None:
        if local_context.dim() == 3:
            local_context = local_context.unsqueeze(0)
        if local_context.dim() != 4:
            raise ValueError(
                f"local_context must be [B,F,L,C_text] or [F,L,C_text], got {tuple(local_context.shape)}"
            )
        b_local, f_local, l_local, _ = local_context.shape
        local_context = rearrange(local_context, "b f l c -> (b f) l c")
        local_context = dit.text_embedding(local_context)
        local_context = rearrange(
            local_context, "(b f) l c -> b f l c", b=b_local, f=f_local
        )

    x = latents

    # Merged CFG / batch alignment follows the shared global condition batch.
    if x.shape[0] != global_context.shape[0]:
        if global_context.shape[0] % x.shape[0] != 0:
            raise ValueError(
                f"Cannot align latent batch {x.shape[0]} with global context batch {global_context.shape[0]}"
            )
        x = torch.cat([x] * (global_context.shape[0] // x.shape[0]), dim=0)
    if timestep.shape[0] != global_context.shape[0]:
        if global_context.shape[0] % timestep.shape[0] != 0:
            raise ValueError(
                f"Cannot align timestep batch {timestep.shape[0]} with context batch {global_context.shape[0]}"
            )
        timestep = torch.cat(
            [timestep] * (global_context.shape[0] // timestep.shape[0]), dim=0
        )

    # VAE image condition remains on the latent input path.
    if y is not None and dit.require_vae_embedding:
        x = torch.cat([x, y], dim=1)

    # CLIP image tokens remain in the shared/global branch.
    # The resulting global residual is added to every video token, so every
    # temporal slot already receives the image prior. Repeating the same
    # first-frame CLIP tokens in the local branch would double-count the image
    # condition and can strengthen unwanted first-frame copying.
    if clip_feature is not None and dit.require_clip_embedding:
        clip_embedding = dit.img_emb(clip_feature)
        global_context = torch.cat([clip_embedding, global_context], dim=1)
    ##### DUAL-CONTEXT END
    ...
    ##### DUAL-CONTEXT START: local slot count must match latent temporal slots
    if local_context is not None:
        assert local_context.shape[1] == f, (
            f"local_context F={local_context.shape[1]} != latent F={f}"
        )
    ##### DUAL-CONTEXT END
    ...
    if hasattr(dit, "wantodance_enable_global") and dit.wantodance_enable_global:
        if wantodance_refimage_feature is not None:
            ...
            global_context = torch.cat([refimage_feature_embedding, global_context], dim=1)
        ...
        if music_feature is not None:
            ...
            if interp_mode == 'bilinear':
                ...
                context_shape_end = global_context.shape[-1] ## 14B 5120
    ...
    if vace_context is not None:
        vace_hints = vace(
            x, vace_context, global_context, t_mod, freqs,
            use_gradient_checkpointing=use_gradient_checkpointing,
            use_gradient_checkpointing_offload=use_gradient_checkpointing_offload
        )
    if tea_cache_update:
                ...
                if use_gradient_checkpointing_offload:
                    with torch.autograd.graph.save_on_cpu():
                        x, x_vap = torch.utils.checkpoint.checkpoint(
                            ...
                            x, global_context, t_mod, freqs, x_vap, context_vap, t_mod_vap, freqs_vap, block_id,
                            use_reentrant=False
                        )
                    ...
                    x, x_vap = vap(block, x, global_context, t_mod, freqs, x_vap, context_vap, t_mod_vap, freqs_vap, block_id)
            else:
                x = gradient_checkpoint_forward(
                    block,
                    use_gradient_checkpointing,
                    use_gradient_checkpointing_offload,
                    ##### DUAL-CONTEXT START: pass both branches into each DiT block
                    x, global_context, local_context, t_mod, freqs
                    ##### DUAL-CONTEXT END
                )
