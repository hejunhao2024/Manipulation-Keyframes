...
class WanVideoUnit_PromptEmbedder(PipelineUnit):
    """
    Text conditioning unit.

    Keyframe mode:
        - Positive branch: each latent keyframe slot uses only its corresponding
          local frame prompt.
        - Negative branch: repeat the negative prompt for every slot.
        - Output shape: [1, F, 512, text_dim].

    Original Wan mode:
        - Encode the ordinary prompt as [B, 512, text_dim].
    """
    ...
    def encode_prompt(self, pipe: WanVideoPipeline, prompt):
        ...
        # Explicitly remove padding-token embeddings.
        ...
        if num_slots is not None:
            if positive:
                if frame_prompts is None:
                    raise ValueError(
                        "frame_prompts must be provided for positive keyframe conditioning"
                    )
                if len(frame_prompts) != num_slots:
                    raise ValueError(
                        f"len(frame_prompts)={len(frame_prompts)} "
                        f"does not match num_slots={num_slots}"
                    )

                # Local-only conditioning: do not append or prepend global prompt.
                slot_prompts = list(frame_prompts)
            else:
                # Here `prompt` is the negative prompt supplied by inputs_nega.
                slot_prompts = [prompt] * num_slots

            prompt_emb = self.encode_prompt(
                pipe,
                slot_prompts,
            )  # [F, 512, text_dim]
            prompt_emb = prompt_emb.unsqueeze(0)  # [1, F, 512, text_dim]
...
def model_fn_wan_video(
    ...
    **kwargs,
    ...
    is_keyframe_context = context is not None and context.dim() == 4

    if is_keyframe_context:
        b_ctx, f_ctx, l_ctx, c_text = context.shape

        context = rearrange(context, "b f l c -> (b f) l c")
        context = dit.text_embedding(context)
        context = rearrange(context, "(b f) l c -> b f l c", b=b_ctx, f=f_ctx)
    else:
        context = dit.text_embedding(context)

    x = latents

    # Merged cfg / batch alignment
    if x.shape[0] != context.shape[0]:
        x = torch.concat([x] * context.shape[0], dim=0)
    if timestep.shape[0] != context.shape[0]:
        timestep = torch.concat([timestep] * context.shape[0], dim=0)
    ...
    # CLIP image condition
    if clip_feature is not None and dit.require_clip_embedding:
        clip_embdding = dit.img_emb(clip_feature)  # [B, 257, C]

        if is_keyframe_context:
            # [B, 257, C] -> [B, F, 257, C]
            clip_embdding = clip_embdding[:, None].expand(-1, f_ctx, -1, -1)

            # Wan original order: [image tokens, text tokens]
            context = torch.cat([clip_embdding, context], dim=2)  # [B, F, 257+512, C]
        else:
            context = torch.cat([clip_embdding, context], dim=1)
    ...
    if is_keyframe_context:
        assert context.shape[1] == f, f"context F={context.shape[1]} != latent F={f}"
        # print("keyframe context:", context.shape, "latent f:", f)
