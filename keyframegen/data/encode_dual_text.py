...
def load_sample_from_dir(sample_dir: str, global_prompt_field: str = "global_prompt_long") -> Dict:
    ...
    global_prompt = ann.get(global_prompt_field) or ann.get("prompt")
...
def build_text_clip_pipe(cfg: Dict, device_override: str = None):
    model_cfg = cfg["model"]
    ...
    pipe = WanVideoPipeline.from_pretrained(
        torch_dtype=dtype,
        device=device,
        model_configs=[text_model_config, image_model_config],
...
def process_bucket(
    ...
    for s in bucket:
        ...
        dual_path = os.path.join(dual_out_dir, f"{s['id']}.pt")
        if os.path.exists(local_path) and os.path.exists(dual_path) and not overwrite:
            skip += 1
        else:
            work.append(s)
    ...
    for start in range(0, len(work), batch_size):
        ...
        try:
            ...
            # Encode global and local text independently.
            # global_context_all: [B, 512, D]
...
def main():
    ...
    samples = [load_sample_from_dir(d, global_prompt_field=global_prompt_field) for d in shard_dirs]

    buckets = bucket_by_num_slots(samples)

    print(f"[mode] global_local_text_to_local_and_dual_caches")
    ...
    pipe = build_text_clip_pipe(cfg, device_override=device)
