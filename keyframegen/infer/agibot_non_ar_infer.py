#!/usr/bin/env python3
"""
AgiBot fixed-length (21-slot) non-autoregressive inference.

Usage:
  Single GPU:
    CUDA_VISIBLE_DEVICES=0 python -u keyframegen/infer/agibot_non_ar_infer.py \
...
GPU selection and process count are intentionally controlled by the launch
command rather than by this script/config.
"""
...
def outputs_complete(sample_output: Path, expected: int) -> bool:
    ...
    return (
        len(files) >= expected
        and (sample_output / "pred_contact.jpg").exists()
...
def validate_config(cfg: Dict[str, Any]) -> None:
    for section in ("model", "checkpoint", "adapter", "data", "inference"):
