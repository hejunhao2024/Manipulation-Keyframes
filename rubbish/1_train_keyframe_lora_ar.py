...
def load_trainable_checkpoint_into_dit(pipe: WanVideoPipeline, path: str, rank: int = 0, strict_resume: bool = True):
    """
    ...
    if strict_resume and missing_shape:
        preview = "\n".join(str(x) for x in missing_shape[:20])
        raise RuntimeError(
            f"Checkpoint is not structurally compatible with the current LoRA/model setup. "
            f"Skipped {len(missing_shape)} tensors. First mismatches:\n{preview}"
        )
...
def train(cfg: Dict):
    ...
    if resume_trainable_path:
        load_trainable_checkpoint_into_dit(
            pipe, resume_trainable_path, rank=rank,
    ...
    for step in range(1, steps + 1):
        ...
        if use_error_recycling:
            with torch.no_grad():
                ...
                should_update = (not use_clean) or (
                    random.random() < clean_buffer_update_prob
                )

                if iteration_count <= warmup_iter:
                    # Every rank MUST enter every collective during warmup.
                    # Otherwise independently sampled should_update values can deadlock DDP.
                    update_flag = torch.tensor(
                        [int(should_update)],
                        device=data_error.device,
                        dtype=torch.int32,
                    )

                    data_list = all_gather_tensor(
                        data_error,
                        distributed,
                        world_size,
                    )
                    noise_list = all_gather_tensor(
                        noise_error,
                        distributed,
                        world_size,
                    )
                    timestep_list = all_gather_tensor(
                        timestep,
                        distributed,
                        world_size,
                    )
                    update_flag_list = all_gather_tensor(
                        update_flag,
                        distributed,
                        world_size,
                    )

                    for de, ne, tt, flag in zip(
                        data_list,
                        noise_list,
                        timestep_list,
                        update_flag_list,
                    ...
                    # No collective after warmup, so local conditional update is safe.
                    if should_update:
