...
def train(cfg: Dict):
    ...
    rank_print(rank, f"[dataset] num samples = {len(samples)}")
    for s in samples:
        rank_print(rank, f"  - {s.get('id', '<no_id>')}: {s['sample_dir'] if 'sample_dir' in s else s.get('image', '')}")

    pipe = build_pipe(cfg)

    # Precompute condition and target latents on each rank.
    train_items = []
    ...
    rank_print(rank, "  num_train_items:", len(train_items))
    ...
    for step in range(1, steps + 1):
        ...
        # Each rank uses a different sample at the same global step.
        item_idx = ((step - 1) * world_size + rank) % len(train_items)
        item = train_items[item_idx]
        sample = item["sample"]
        sample_id = sample.get("id", f"sample_{item_idx}")
        cond = item["cond"]
        ...
        if is_rank0(rank) and step % log_every == 0:
            print(f"[step {step:06d}] loss={loss_for_log.item():.6f} sample_rank0={sample_id}")
            writer.add_scalar("train/loss", loss_for_log.item(), step)
            writer.add_scalar(f"sample_loss_rank0/{sample_id}", loss_detached.item(), step)
        ...
        if preview_every > 0 and (step % preview_every == 0 or step == steps):
            ...
            if is_rank0(rank):
                try:
                    preview_sample = train_items[0]["sample"]
                    preview_dir = save_preview(pipe, preview_sample, infer_cfg, out_dir, step)
