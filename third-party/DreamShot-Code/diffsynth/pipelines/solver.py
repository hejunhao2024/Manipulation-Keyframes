...
def euler_step(
    ...
    # 核心安全机制：如果到了最后一步，下一步的 sigma 强行设为 0
    if index + 1 >= len(sigmas):
        sigma_next = torch.tensor(0.0, dtype=sigma.dtype, device=sigma.device)
    else:
        sigma_next = sigmas[index + 1]
        
    # Euler Update
    prev_sample = latents + model_output * (sigma_next - sigma)
    
    # 保持和其他函数一样的返回值格式 (由于是确定性采样，log_prob 为 None)
    return prev_sample, None, None


def run_sampling_step(
    pred,               # 外部算好的 noise_pred
    z,                  # 当前的 latents
    sigma_schedule,
    step_index,         # 当前的 step index (对应外层的 progress_id)
    solver="flow",
    determistic=False,
    eta=0.7,
    dpm_state=None,     # 外部传入的 DPM 状态
):
    assert solver in ["flow", "dance", "ddim", "dpm1", "dpm2", "euler"]
    dtype = z.dtype

    if solver == "euler":
        z, pred_original, log_prob = euler_step(
        ...
        z, pred_original, log_prob = dance_grpo_step(
            pred.float(), z.float(), eta if not determistic else 0, sigmas=sigma_schedule, index=step_index, prev_sample=None
        )
    elif solver == "ddim":
        ...
        assert determistic
        assert dpm_state is not None, "Using DPM solver requires passing an initialized dpm_state."
        order = int(solver[-1])
